...
$all_app = Rails.application.config_for(:application)

class Object
  require 'set'

  BLACKLIST_METHODS = %w[eval instance_eval send __send__ public_send exit exit! abort system exec fork open sleep
                         spawn syscall ` throw].to_set.freeze
